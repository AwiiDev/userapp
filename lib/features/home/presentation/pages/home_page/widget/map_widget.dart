  ...
  Widget build(BuildContext context) {
    return GoogleMap(
        ...
        initialCameraPosition: (homeBloc.initialCameraPosition !=
            ...
            : CameraPosition(
                target:
                    homeBloc.currentLatLng ?? const LatLng(0, 0),
                zoom: 15.0,
              ),
...
class _AutoCompletionEmptyView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final size = MediaQuery.sizeOf(context);
    return Positioned(
        right: (homeBloc.textDirection == 'rtl')
            ? null
            : size.width * 0.05,
        left: (homeBloc.textDirection == 'ltr')
            ? null
            : size.width * 0.05,
    ...
    return fm.FlutterMap(
      ...
      children: [
        ...
        if (userData != null && userData!.role == 'owner')
          fm.MarkerLayer(
            markers:
                List.generate(homeBloc.markers.length, (index) {
