...
class DriverProfileBloc extends Bloc<DriverProfileEvent, DriverProfileState> {
  ...
  DriverProfileBloc() : super(DriverProfileInitialState()) {
    ...
    on<GetVehicleModelEvent>(getVehicleModel);
    on<UpdateVehicleTypeEvent>(updateVehicleType);
    on<UpdateVehicleMakeEvent>(updateVehicleMake);
    on<UpdateVehicleModelEvent>(updateVehicleModel);
    on<UpdateVehicleColorEvent>(updateVehicleColor);
  ...
  FutureOr<void> getVehicleType(
    ...
    if (userData!.role == 'driver' || event.from == 'owner') {
      ...
      data.fold(
        ...
        (success) {
          vehicleType = success['types'].data;
          vehicleMakes = success['makes'];
          vehicleColors = success['colors'];
        },
      );
    }
    emit(LoadingStopState());
  ...
  FutureOr<void> getVehicleModel(
      GetVehicleModelEvent event, Emitter<DriverProfileState> emit) async {
    emit(LoadingStartState());

      final data = await serviceLocator<DriverProfileUsecase>()
          .getVehicleModel(id: event.id);
      data.fold(
            (error) {
          emit(ShowErrorState(message: error.message.toString()));
        },
            (success) {
          vehicleModels = success;
        },
      );
  ...
  FutureOr<void> updateVehicleMake(
      UpdateVehicleMakeEvent event, Emitter<DriverProfileState> emit) async {
    customMake = event.id;
    customModel = null;
    vehicleYear.clear();
    vehicleNumber.clear();
  ...
  FutureOr<void> updateVehicleModel(
      UpdateVehicleModelEvent event, Emitter<DriverProfileState> emit) async {
    customModel = event.id;
    vehicleYear.clear();
    vehicleNumber.clear();
    vehicleColor = null;
    emit(DriverUpdateState());
  ...
  FutureOr<void> updateVehicleColor(
      UpdateVehicleColorEvent event, Emitter<DriverProfileState> emit) async {
    vehicleColor = event.id;
    emit(DriverUpdateState());
  }
  ...
  FutureOr<void> updateVehicle(
    ...
    final data = await serviceLocator<DriverProfileUsecase>().updateVehicle(
        ...
        customModel: customModel.toString(),
        carColor: vehicleColor.toString(),
